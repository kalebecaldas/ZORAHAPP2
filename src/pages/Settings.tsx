...
const Settings: React.FC = () => {
  ...
  const [insuranceForm, setInsuranceForm] = useState({ code: '', name: '', displayName: '', sessionPrice: 0, packagePrice: 0, packageSessions: 0, packageSessionsStr: '', isActive: true, notes: '' });
  ...
  const fetchEditableData = async () => {
    try {
      ...
      clinicsData.forEach((c: any) => { mapByClinic[c.code] = Array.isArray(c.insurances) ? c.insurances : []; });
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {clinics.map((clinic) => (
                <div key={clinic.code} className="border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow">
                  ...
                  <div className="mb-3">
                    <p className="text-xs text-gray-500 mb-1">Convênios vinculados:</p>
                    <div className="flex flex-wrap gap-1">
                      {(Array.isArray(clinicInsurances[clinic.code]) ? clinicInsurances[clinic.code] : []).slice(0,3).map((ci: any, index: number) => (
                        <span key={index} className="text-xs bg-indigo-100 text-indigo-800 px-2 py-1 rounded">
                          {ci?.displayName || ci?.name || ci?.insuranceCode}
                        </span>
                      ))}
                      {Array.isArray(clinicInsurances[clinic.code]) && (clinicInsurances[clinic.code] || []).length > 3 && (
                        <span className="text-xs text-gray-500">+{(clinicInsurances[clinic.code] || []).length - 3} mais</span>
                      )}
                    </div>
                  </div>
                <div className="flex space-x-2">
              ...
              <tbody>
                {(procSortAsc ? [...procedures] : [...procedures].reverse())
                  ...
                  <tr key={p.code} className="border-t">
                    ...
                    <td className="py-2">
                      <textarea value={p.description} onChange={(e)=> setProcedures(prev => prev.map(x => x.code===p.code ? { ...x, description: e.target.value } : x))} className="border rounded-md px-2 py-1 w-64 h-20" />
                    </td>
                    <td className="py-2 space-x-2">
                      <button onClick={() => updateProcedure(p.code, { code: p.code, name: p.name, description: p.description, basePrice: 0, requiresEvaluation: false, duration: Number(p.duration||30), categories: curatedCategoryByName[p.name] ? [curatedCategoryByName[p.name]] : [] })} className="px-3 py-1 border rounded-md">Salvar</button>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-30 flex items-center justify-center z-50" role="dialog" aria-modal="true">
          <div className="bg-white rounded-lg shadow-lg w-full max-w-2xl p-6">
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={()=>setEditingInsurance(null)} className="p-2 text-gray-500 hover:text-gray-700"><X className="h-4 w-4" /></button>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6 max-h-[80vh] overflow-y-auto">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Código</label>
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Nome</label>
                <input value={editingInsurance.name} onChange={(e)=>setEditingInsurance({ ...editingInsurance, name: e.target.value })} className={`w-full border rounded-md px-3 py-2 ${!editingInsurance.name ? 'border-red-400' : 'border-gray-300'}`} aria-invalid={!editingInsurance.name} aria-required="true" />
              </div>
              <div>
            ...
            <div className="border-t pt-4">
              <div className="flex items-center justify-between mb-3">
                ...
                <div className="flex items-center space-x-2">
                  <select value={cipClinicSelection} onChange={(e)=>{ e.stopPropagation(); const v=e.target.value; setCipClinicSelection(v); }} className="border rounded-md px-3 py-2 text-sm">
                    <option value="">Selecione a clínica</option>
                    {clinics.map(c => (<option key={c.code} value={c.code}>{c.displayName || c.name}</option>))}
                  </select>
                  <button onClick={()=>{ setCipSortBy(b=> b==='name' ? 'price' : 'name'); }} className="px-3 py-2 border rounded-md text-sm">Ordenar por {cipSortBy==='name'?'preço':'nome'}</button>
                  <button onClick={()=>setCipSortAsc(a=>!a)} className="px-3 py-2 border rounded-md text-sm">{cipSortAsc?'Asc':'Desc'}</button>
                </div>
              </div>
              {cipClinicSelection && (
                <div>
                  <div className="flex items-center mb-3">
                    {cipLoading && (<Loader2 className="h-4 w-4 animate-spin text-gray-500" />)}
                  </div>
                  <div className="mb-3 flex items-center space-x-2">
                    <select value={newClinicInsuranceProcedure.procedureCode} onChange={e => setNewClinicInsuranceProcedure({ ...newClinicInsuranceProcedure, procedureCode: e.target.value })} className="border p-2 rounded-md text-sm">
                      <option value="">Selecione</option>
                      {Object.entries(curatedProceduresCatalog.reduce((acc: Record<string, string[]>, item) => {
                        (acc[item.category] ||= []).push(item.name);
                        return acc;
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                      {clinics.map(c => (
                        <label key={c.code} className="inline-flex items-center space-x-1 text-xs border rounded px-2 py-1">
                          <input type="checkbox" checked={selectedClinicsForNewProc.includes(c.code)} onChange={(e)=>{
                    ...
                    <input type="text" inputMode="numeric" placeholder="Preço por sessão" value={formatBRL(Number(newClinicInsuranceProcedure.price || 0))} onChange={e => setNewClinicInsuranceProcedure({ ...newClinicInsuranceProcedure, price: parseBRLInput(e.target.value) })} className="border p-2 rounded-md text-sm" />
                    <label className="flex items-center space-x-2 text-sm"><input type="checkbox" checked={newClinicInsuranceProcedure.isActive} onChange={e => setNewClinicInsuranceProcedure({ ...newClinicInsuranceProcedure, isActive: e.target.checked })} /><span>Ativo</span></label>
                    <label className="flex items-center space-x-2 text-sm"><input type="checkbox" checked={newClinicInsuranceProcedure.hasPackage} onChange={e => setNewClinicInsuranceProcedure({ ...newClinicInsuranceProcedure, hasPackage: e.target.checked })} /><span>Pacote</span></label>
                    {newClinicInsuranceProcedure.hasPackage && (
                      <div className="flex items-center space-x-2">
                        <input type="text" inputMode="numeric" placeholder="Qtd. Sessões (Pacote)" value={String(newClinicInsuranceProcedure.packageSessions || 0) === '0' ? '' : String(newClinicInsuranceProcedure.packageSessions)} onChange={e => { const d = e.target.value.replace(/\D/g,''); setNewClinicInsuranceProcedure({ ...newClinicInsuranceProcedure, packageSessions: Number(d || 0) }) }} className="border p-2 rounded-md text-sm" />
                        <input type="text" inputMode="numeric" placeholder="Valor do Pacote" value={formatBRL(Number(newClinicInsuranceProcedure.packagePrice || 0))} onChange={e => setNewClinicInsuranceProcedure({ ...newClinicInsuranceProcedure, packagePrice: parseBRLInput(e.target.value) })} disabled={Number(newClinicInsuranceProcedure.packageSessions || 0) <= 1} className="border p-2 rounded-md text-sm disabled:opacity-50 disabled:cursor-not-allowed" />
                      </div>
                    )}

  
                    <button onClick={()=> saveClinicInsuranceProcedureMulti(selectedClinicsForNewProc, editingInsurance.code, newClinicInsuranceProcedure)} className="bg-blue-600 text-white px-3 py-2 rounded-md text-sm">Adicionar</button>
                  </div>
                  <table className="min-w-full text-sm">
                    <thead>
                      <tr className="text-left text-gray-600">
                    ...
                    <tbody>
                      {(cipSortAsc ? [...clinicInsuranceProcedures] : [...clinicInsuranceProcedures].reverse())
                        ...
                        <tr key={p.procedureCode} className="border-t">
                          <td className="py-2">{p.name || p.procedureCode}</td>
                          <td className="py-2">Convênio</td>
                          <td className="py-2">
                            <input type="text" inputMode="numeric" value={formatBRL(Number(p.price||0))} onChange={e=>{
                              const v = parseBRLInput(e.target.value);
                              setClinicInsuranceProcedures(prev => prev.map(x => x.procedureCode===p.procedureCode ? { ...x, price: v } : x));
                            }} className="border rounded-md px-2 py-1 w-32" />
                          </td>
                          <td className="py-2 space-x-2">
                            <label className="inline-flex items-center space-x-1 text-xs"><input type="checkbox" checked={!!p.isActive} onChange={e=> setClinicInsuranceProcedures(prev => prev.map(x => x.procedureCode===p.procedureCode ? { ...x, isActive: e.target.checked } : x))} /><span>Ativo</span></label>
                            <label className="inline-flex items-center space-x-1 text-xs"><input type="checkbox" checked={!!p.hasPackage} onChange={e=> setClinicInsuranceProcedures(prev => prev.map(x => x.procedureCode===p.procedureCode ? { ...x, hasPackage: e.target.checked } : x))} /><span>Pacote</span></label>
                            {p.hasPackage && (
                              <span className="inline-flex items-center space-x-2">
                                <input type="text" inputMode="numeric" value={String(p._packageSessions || 0) === '0' ? '' : String(p._packageSessions)} onChange={e=>{
                                  const d = e.target.value.replace(/\D/g,'');
                                  setClinicInsuranceProcedures(prev => prev.map(x => x.procedureCode===p.procedureCode ? { ...x, _packageSessions: Number(d || 0) } : x));
                                }} className="border rounded-md px-2 py-1 text-xs w-24" placeholder="Qtd. sessões" />
                                <input type="text" inputMode="numeric" value={formatBRL(Number(p._packagePrice || 0))} onChange={e=>{
                                  const val = parseBRLInput(e.target.value);
                                  setClinicInsuranceProcedures(prev => prev.map(x => x.procedureCode===p.procedureCode ? { ...x, _packagePrice: val } : x));
                                }} disabled={Number(p._packageSessions || 0) <= 1} className="border rounded-md px-2 py-1 text-xs w-28 disabled:opacity-50 disabled:cursor-not-allowed" placeholder="R$ 0,00" />
                              </span>
                            )}
                            <button onClick={() => saveClinicInsuranceProcedure(cipClinicSelection, editingInsurance.code, p)} className="px-2 py-1 border rounded-md text-xs">Salvar</button>
                            <button onClick={() => deleteClinicInsuranceProcedure(cipClinicSelection, editingInsurance.code, p.procedureCode)} className="px-2 py-1 border rounded-md text-xs text-red-600">Remover</button>
                          </td>
                        </tr>
                      ))}
                    </tbody>
            ...
            <div className="flex items-center justify-end space-x-2 mt-6">
              <button type="button" onClick={()=>setEditingInsurance(null)} className="px-4 py-2 border rounded-md">Cancelar</button>
              <button type="button" onClick={async ()=>{
                const payload = { code: editingInsurance.code, name: editingInsurance.name, displayName: editingInsurance.displayName, isActive: !!editingInsurance.isActive };
                await updateInsurance(editingInsurance.code, payload);
                setInsuranceEditHistory(prev => ([...prev, { action: 'editar', at: new Date().toISOString() }]));
                // Mantém o modal aberto e atualiza a lista sem navegar
                fetchEditableData();
                setTimeout(() => setEditingInsurance(null), 300);
                ...
                {newInsuranceClinicSelection && (
                  <div>
                    <div className="mb-3 flex items-center space-x-2">
                      ...
                      <input type="text" inputMode="numeric" placeholder="Preço por sessão" value={formatBRL(Number(newInsuranceProcedure.price || 0))} onChange={e => setNewInsuranceProcedure({ ...newInsuranceProcedure, price: parseBRLInput(e.target.value) })} className="border p-2 rounded-md text-sm" />
                      ...
                      <label className="flex items-center space-x-2 text-sm"><input type="checkbox" checked={newInsuranceProcedure.hasPackage} onChange={e => setNewInsuranceProcedure({ ...newInsuranceProcedure, hasPackage: e.target.checked })} /><span>Pacote</span></label>
                      {newInsuranceProcedure.hasPackage && (
                        <div className="flex items-center space-x-2">
                          <input type="text" inputMode="numeric" placeholder="Qtd. Sessões (Pacote)" value={String(newInsuranceProcedure.packageSessions || 0) === '0' ? '' : String(newInsuranceProcedure.packageSessions)} onChange={e => { const d = e.target.value.replace(/\D/g,''); setNewInsuranceProcedure({ ...newInsuranceProcedure, packageSessions: Number(d || 0) }) }} className="border p-2 rounded-md text-sm" />
                          <input type="text" inputMode="numeric" placeholder="Valor do Pacote" value={formatBRL(Number(newInsuranceProcedure.packagePrice || 0))} onChange={e => setNewInsuranceProcedure({ ...newInsuranceProcedure, packagePrice: parseBRLInput(e.target.value) })} disabled={Number(newInsuranceProcedure.packageSessions || 0) <= 1} className="border p-2 rounded-md text-sm disabled:opacity-50 disabled:cursor-not-allowed" />
                        </div>
                      )}
                      <button onClick={()=>{
            ...
            <div className="flex items-center justify-end space-x-2 mt-6">
              ...
              <button onClick={async ()=>{
                try {
                  ...
                  setInsuranceCreateOpen(false);
                  setInsuranceForm({ code: '', name: '', displayName: '', sessionPrice: 0, packagePrice: 0, packageSessions: 0, packageSessionsStr: '', isActive: true, notes: '' });
              ...
              <div className="flex items-center justify-between mb-3">
                ...
                <div className="flex items-center space-x-2">
                  {(() => {
                    ...
                    const filtered = sorted.filter(ic => (ic.displayName || ic.name || '').toLowerCase().includes(insMultiQuery.toLowerCase()));
                    const labelByCode = (code: string) => {
                      const ic = insurances.find(i => i.code === code);
                      return ic?.displayName || ic?.name || code;
                    };
                    return (
                      <div className="relative">
                        <div className="flex flex-wrap gap-1 mb-2 max-w-[360px]">
                          {linkClinicInsuranceCodes.map(code => (
                            <span key={code} className="text-xs bg-indigo-100 text-indigo-800 px-2 py-1 rounded inline-flex items-center">
                              {labelByCode(code)}
                              <button type="button" onClick={() => setLinkClinicInsuranceCodes(prev => prev.filter(c => c!==code))} className="ml-1 text-indigo-700">×</button>
                            </span>
                          ))}
                        </div>
                        <div className="border rounded-md px-3 py-2 text-sm min-w-[260px]">
                          <input type="text" value={insMultiQuery} onChange={e=> setInsMultiQuery(e.target.value)} placeholder="Filtrar convênios" className="w-full outline-none border-none" />
                          <div className="mt-2 flex items-center justify-end">
                            <button type="button" onClick={() => {
                              const allCodes = filtered.map(i=> i.code);
                              const next = Array.from(new Set([...linkClinicInsuranceCodes, ...allCodes]));
                              setLinkClinicInsuranceCodes(next);
                            }} className="text-xs px-2 py-1 bg-indigo-600 text-white rounded">Selecionar todos</button>
                          </div>
                          <div className="mt-2 max-h-40 overflow-y-auto">
                            {filtered.map(ic => {
                              const selected = linkClinicInsuranceCodes.includes(ic.code);
                              return (
                                <button key={ic.code} type="button" onClick={() => setLinkClinicInsuranceCodes(prev => selected ? prev.filter(c => c!==ic.code) : [...prev, ic.code])} className={`w-full text-left px-2 py-1 rounded ${selected ? 'bg-indigo-50 text-indigo-700' : 'hover:bg-gray-100'}`}>
                  ...
                  <button onClick={async ()=>{
                    try {
                      if (!editingClinic || !editingClinic.code) { toast.error('Selecione a clínica'); return; }
                      const codes = linkClinicInsuranceCodes.filter(Boolean);
                      if (codes.length === 0) { toast.error('Selecione ao menos um convênio'); return; }
                      for (const code of codes) {
                        await api.post(`/api/clinic/clinics/${editingClinic.code}/insurances/${code}`, { active: true });
                      }
                      ...
                      toast.success('Convênios vinculados');
                    } catch { toast.error('Erro ao vincular convênios'); }
                  }} className="text-xs bg-indigo-600 text-white px-3 py-2 rounded">Adicionar selecionados</button>
                </div>
              </div>
