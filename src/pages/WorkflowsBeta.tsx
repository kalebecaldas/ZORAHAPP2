...
export const WorkflowsBeta: React.FC = () => {
  ...
  const [workflow, setWorkflow] = useState<{ id?: string; name: string; description: string; nodes: WorkflowNode[]; isActive?: boolean }>({
    name: 'Workflow Beta',
  ...
  const handleSave = async (wfData: { name?: string; description?: string; nodes: WorkflowNode[]; edges?: unknown; isActive?: boolean }) => {
    try {
